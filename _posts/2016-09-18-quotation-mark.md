...
-------| -----|                :----:      |-----
U+2019<br> _Right Single Quotation Mark_                | `’` | `AltGr` + `ъ`          | Помимо апострофа (см. выше), символ используется в английском тексте как **закрывающая** кавычка внутри других кавычек.
...
U+201E<br> _Double Low-9 Quotation Mark_                | `„` | `AltGr` + `л`          | Нижняя открывающая двойная кавычка. <br>Русский язык — открывающие кавычки внутри других кавычек или при письме от руки. <br>Английский язык — не используется.
U+201C<br> _Left Double Quotation Mark_                 | `“` | `AltGr` + `д`          | Закрывающая двойная кавычка. <br>Русский язык — закрывающие кавычки внутри других кавычек и при письме от руки. <br>Английский язык — открывающие кавычки.
...
* *(немецкий)* снаружи используете то же, что и внутри на русском — двойные 96. 96 — как немецкий [U-96](https://ru.wikipedia.org/wiki/U-96_(1940)). А правильные внутренние кавычки на немецком набрать не получится, хотя если вместо U+201A *Single Low-9 Quotation Mark* `‚` использовать запятую, то можно, см. пример.
